...
require "#{$script_dir}/lib/media-sorter-music"
...
begin
  ...
  puts "#{@script} -> yaml error #{e.message} in media-sorter.yaml.default"  
  exit 2
end
...
if File.exists? custom_config
  begin
    ...
    puts "#{@script} -> yaml error #{e.message} in #{custom_config}"  
    exit 2
  end
